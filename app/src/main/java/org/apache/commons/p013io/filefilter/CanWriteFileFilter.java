package org.apache.commons.p013io.filefilter;

import java.io.File;
import java.io.Serializable;

/* JADX INFO: loaded from: classes2.dex */
public class CanWriteFileFilter extends AbstractFileFilter implements Serializable {

    /* JADX INFO: renamed from: a */
    public static final long f15723a = 5132005214688990379L;

    /* JADX INFO: renamed from: b */
    public static final IOFileFilter f15724b;

    /* JADX INFO: renamed from: c */
    public static final IOFileFilter f15725c;

    static {
        CanWriteFileFilter canWriteFileFilter = new CanWriteFileFilter();
        f15724b = canWriteFileFilter;
        f15725c = new NotFileFilter(canWriteFileFilter);
    }

    /* JADX WARN: Invalid debug info offset */
    @Override // org.apache.commons.p013io.filefilter.AbstractFileFilter, org.apache.commons.p013io.filefilter.IOFileFilter, java.io.FileFilter
    public boolean accept(File file) {
        return false;
    }
}
