...
final class MultiDexExtractor implements Closeable {
    ...
    public MultiDexExtractor(File file, File file2) throws Throwable {
        Log.i(TAG, "MultiDexExtractor(" + file.getPath() + ", " + file2.getPath() + C1313UP.f3459l);
        this.sourceApk = file;
        this.dexDir = file2;
    ...
    private static void extract(ZipFile zipFile, ZipEntry zipEntry, File file, String str) throws IOException {
        ...
        File fileCreateTempFile = File.createTempFile(C5312rZ.m27869a("tmp-", str), ".zip", file.getParentFile());
    ...
    private List<ExtractedDex> loadExistingExtractions(Context context, String str) throws IOException {
        ...
        while (i2 <= i) {
            ...
            if (!extractedDex.isFile()) {
                throw new IOException("Missing extracted secondary dex file '" + extractedDex.getPath() + C1313UP.f3457j);
            }
                ...
                while (i2 < 3 && !z) {
                    ...
                    if (!z) {
                        ...
                        if (extractedDex.exists()) {
                            Log.w(TAG, "Failed to delete corrupted secondary dex '" + extractedDex.getPath() + C1313UP.f3457j);
                        }
                    }
                }
                if (!z) {
                    throw new IOException("Could not create zip file " + extractedDex.getAbsolutePath() + " for secondary dex (" + i + C1313UP.f3459l);
                }
                i++;
    ...
    private static void putStoredApkInfo(Context context, String str, long j, long j2, List<ExtractedDex> list) {
        ...
        editorEdit.putLong(C0786MA.m3668a(new StringBuilder(), str, KEY_CRC), j2);
    ...
    public List<? extends File> load(Context context, String str, boolean z) throws IOException {
        ...
        Log.i(TAG, "MultiDexExtractor.load(" + this.sourceApk.getPath() + ", " + z + ", " + str + C1313UP.f3459l);
