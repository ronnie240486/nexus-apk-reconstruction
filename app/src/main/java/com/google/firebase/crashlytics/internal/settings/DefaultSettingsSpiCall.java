...
class DefaultSettingsSpiCall implements SettingsSpiCall {
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            this.logger.m17659w("Failed to parse settings JSON from " + this.url, e);
            this.logger.m17658w("Settings response " + str);
            return null;
        }
    }
    ...
    public JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        this.logger.m17656v("Settings response code was: " + iCode);
        if (requestWasSuccessful(iCode)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        Logger logger = this.logger;
        StringBuilder sbM27771a = C5274qy.m27771a("Settings request failed; (status: ", iCode, ") from ");
        sbM27771a.append(this.url);
        logger.m17652e(sbM27771a.toString());
        return null;
    }
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        ...
        try {
            ...
            this.logger.m17650d("Requesting settings from " + this.url);
            this.logger.m17656v("Settings query params were: " + queryParamsFor);
            return handleResponse(httpGetRequestApplyHeadersTo.execute());
        } catch (IOException e) {
            this.logger.m17653e("Settings request failed.", e);
            return null;
        }
    }
