...
public final class VCardResultParser extends ResultParser {
    ...
    private static final Pattern COMMA = Pattern.compile(C1262Tc.f3278g);
            ...
            if (iIndexOf < 0) {
                ...
                if (iIndexOf > iEnd) {
                    ...
                    if (z3) {
                        ...
                        if (z2) {
                            strReplaceAll = UNESCAPED_SEMICOLONS.matcher(strReplaceAll).replaceAll(IOUtils.f15646e).trim();
                        }
                    } else {
                        if (z2) {
                            strSubstring = UNESCAPED_SEMICOLONS.matcher(strSubstring).replaceAll(IOUtils.f15646e).trim();
                        }
                        strReplaceAll = VCARD_ESCAPES.matcher(NEWLINE_ESCAPE.matcher(CR_LF_SPACE_TAB.matcher(strSubstring).replaceAll("")).replaceAll(IOUtils.f15646e)).replaceAll("$1");
                    }
                    if (arrayList == null) {
