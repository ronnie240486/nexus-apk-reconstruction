...
public final class Mp4Extractor implements Extractor, SeekMap {
    ...
    public static final ExtractorsFactory FACTORY = new ExtractorsFactory() { // from class: ZC
    ...
    private ArrayList<TrackSampleTable> getTrackSampleTables(Atom.ContainerAtom containerAtom, GaplessInfoHolder gaplessInfoHolder, boolean z) throws ParserException {
        ...
        for (int i = 0; i < containerAtom.containerChildren.size(); i++) {
            ...
            if (containerAtom2.type == 1953653099 && (trak = AtomParsers.parseTrak(containerAtom2, containerAtom.getLeafAtomOfType(Atom.TYPE_mvhd), C3219C.TIME_UNSET, null, z, this.isQuickTime)) != null) {
    ...
    private boolean readAtomHeader(ExtractorInput extractorInput) throws InterruptedException, IOException {
        ...
        if (j3 != 1) {
            ...
            if (shouldParseContainerAtom(this.atomType)) {
                ...
                j = this.atomSize;
                i = this.atomHeaderBytesRead;
                j2 = (position2 + j) - ((long) i);
                if (j != i && this.atomType == 1835365473) {
                    maybeSkipRemainingMetaAtomHeaderBytes(extractorInput);
                }
                this.containerAtoms.push(new Atom.ContainerAtom(this.atomType, j2));
                if (this.atomSize == this.atomHeaderBytesRead) {
                    processAtomEnded(j2);
                } else {
                    enterReadingAtomHeaderState();
        ...
        if (shouldParseContainerAtom(this.atomType)) {
            ...
            if (shouldParseLeafAtom(this.atomType)) {
                ...
                Assertions.checkState(z2);
                ParsableByteArray parsableByteArray2 = new ParsableByteArray((int) this.atomSize);
    ...
    private int readSample(ExtractorInput extractorInput, PositionHolder positionHolder) throws InterruptedException, IOException {
        ...
        trackOutput.sampleMetadata(trackSampleTable2.timestampsUs[i], trackSampleTable2.flags[i], i8, 0, null);
    ...
    public SeekMap.SeekPoints getSeekPoints(long j) {
        ...
        return j3 == C3219C.TIME_UNSET ? new SeekMap.SeekPoints(seekPoint) : new SeekMap.SeekPoints(seekPoint, new SeekPoint(j3, jMaybeAdjustSeekOffset));
    }
