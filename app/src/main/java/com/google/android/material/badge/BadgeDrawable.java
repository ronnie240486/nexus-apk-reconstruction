...
public class BadgeDrawable extends Drawable implements TextDrawableHelper.TextDrawableDelegate {
    ...
    @StyleRes
    private static final int DEFAULT_STYLE = C3387R.style.Widget_MaterialComponents_Badge;

    @AttrRes
    private static final int DEFAULT_THEME_ATTR = C3387R.attr.badgeStyle;
    ...
    public static final class SavedState implements Parcelable {
        ...
        public SavedState(@NonNull Context context) {
            this.alpha = 255;
            this.number = -1;
            this.badgeTextColor = new TextAppearance(context, C3387R.style.TextAppearance_MaterialComponents_Badge).textColor.getDefaultColor();
            this.contentDescriptionNumberless = context.getString(C3387R.string.mtrl_badge_numberless_content_description);
            this.contentDescriptionQuantityStrings = C3387R.plurals.mtrl_badge_content_description;
            this.contentDescriptionExceedsMaxBadgeNumberRes = C3387R.string.mtrl_exceed_max_badge_number_content_description;
            this.isVisible = true;
        }
    ...
    private BadgeDrawable(@NonNull Context context) {
        ...
        this.badgeRadius = resources.getDimensionPixelSize(C3387R.dimen.mtrl_badge_radius);
        this.badgeWidePadding = resources.getDimensionPixelSize(C3387R.dimen.mtrl_badge_long_text_horizontal_padding);
        this.badgeWithTextRadius = resources.getDimensionPixelSize(C3387R.dimen.mtrl_badge_with_text_radius);
        TextDrawableHelper textDrawableHelper = new TextDrawableHelper(this);
        this.textDrawableHelper = textDrawableHelper;
        textDrawableHelper.getTextPaint().setTextAlign(Paint.Align.CENTER);
        this.savedState = new SavedState(context);
        setTextAppearanceResource(C3387R.style.TextAppearance_MaterialComponents_Badge);
    }
    ...
    private void calculateCenterAndBounds(@NonNull Context context, @NonNull Rect rect, @NonNull View view) {
        ...
        if (i == 8388691 || i == 8388693) {
            f = rect.bottom - this.savedState.verticalOffset;
        } else {
            f = this.savedState.verticalOffset + rect.top;
        }
        this.badgeCenterY = f;
        if (getNumber() <= 9) {
        ...
        int dimensionPixelSize = context.getResources().getDimensionPixelSize(hasNumber() ? C3387R.dimen.mtrl_badge_text_horizontal_edge_offset : C3387R.dimen.mtrl_badge_horizontal_edge_offset);
        int i2 = this.savedState.badgeGravity;
        this.badgeCenterX = (i2 == 8388659 || i2 == 8388691 ? ViewCompat.getLayoutDirection(view) != 0 : ViewCompat.getLayoutDirection(view) == 0) ? ((rect.right + this.halfBadgeWidth) - dimensionPixelSize) - this.savedState.horizontalOffset : (rect.left - this.halfBadgeWidth) + dimensionPixelSize + this.savedState.horizontalOffset;
    }
    ...
    private String getBadgeText() {
        ...
        return context == null ? "" : context.getString(C3387R.string.mtrl_exceed_max_badge_number_suffix, Integer.valueOf(this.maxBadgeNumber), "+");
    }

    private void loadDefaultStateFromAttributes(Context context, AttributeSet attributeSet, @AttrRes int i, @StyleRes int i2) {
        TypedArray typedArrayObtainStyledAttributes = ThemeEnforcement.obtainStyledAttributes(context, attributeSet, C3387R.styleable.Badge, i, i2, new int[0]);
        setMaxCharacterCount(typedArrayObtainStyledAttributes.getInt(C3387R.styleable.Badge_maxCharacterCount, 4));
        if (typedArrayObtainStyledAttributes.hasValue(C3387R.styleable.Badge_number)) {
            setNumber(typedArrayObtainStyledAttributes.getInt(C3387R.styleable.Badge_number, 0));
        }
        setBackgroundColor(readColorFromAttributes(context, typedArrayObtainStyledAttributes, C3387R.styleable.Badge_backgroundColor));
        if (typedArrayObtainStyledAttributes.hasValue(C3387R.styleable.Badge_badgeTextColor)) {
            setBadgeTextColor(readColorFromAttributes(context, typedArrayObtainStyledAttributes, C3387R.styleable.Badge_badgeTextColor));
        }
        setBadgeGravity(typedArrayObtainStyledAttributes.getInt(C3387R.styleable.Badge_badgeGravity, TOP_END));
        setHorizontalOffset(typedArrayObtainStyledAttributes.getDimensionPixelOffset(C3387R.styleable.Badge_horizontalOffset, 0));
        setVerticalOffset(typedArrayObtainStyledAttributes.getDimensionPixelOffset(C3387R.styleable.Badge_verticalOffset, 0));
    ...
    private void tryWrapAnchorInCompatParent(final View view) {
        ...
        if (viewGroup == null || viewGroup.getId() != C3387R.id.mtrl_anchor_parent) {
            WeakReference<FrameLayout> weakReference = this.customBadgeParentRef;
            if (weakReference == null || weakReference.get() != viewGroup) {
                updateAnchorParentToNotClip(view);
                final FrameLayout frameLayout = new FrameLayout(view.getContext());
                frameLayout.setId(C3387R.id.mtrl_anchor_parent);
