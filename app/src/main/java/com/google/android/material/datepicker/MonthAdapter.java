...
class MonthAdapter extends BaseAdapter {
    ...
    private void updateSelectedState(@Nullable TextView textView, long j) {
        ...
        if (this.calendarConstraints.getDateValidator().isValid(j)) {
            ...
            if (isSelected(j)) {
                ...
                calendarItemStyle = timeInMillis == j ? calendarStyle.todayDay : calendarStyle.day;
            }
        } else {
    ...
    public TextView getView(int i, @Nullable View view, @NonNull ViewGroup viewGroup) {
        ...
        if (view == null) {
            textView = (TextView) LayoutInflater.from(viewGroup.getContext()).inflate(C3387R.layout.mtrl_calendar_day, viewGroup, false);
        }
        ...
        if (iFirstPositionInMonth >= 0) {
            ...
            if (iFirstPositionInMonth >= month.daysInMonth) {
                ...
                textView.setContentDescription(this.month.year == Month.current().year ? DateStrings.getMonthDayOfWeekDay(day) : DateStrings.getYearMonthDayOfWeekDay(day));
