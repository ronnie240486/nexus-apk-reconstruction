...
public class TooltipDrawable extends MaterialShapeDrawable implements TextDrawableHelper.TextDrawableDelegate {
    ...
    @StyleRes
    private static final int DEFAULT_STYLE = C3387R.style.Widget_MaterialComponents_Tooltip;

    @AttrRes
    private static final int DEFAULT_THEME_ATTR = C3387R.attr.tooltipStyle;
    ...
    private EdgeTreatment createMarkerEdge() {
        ...
        Double.isNaN(d);
        Double.isNaN(dWidth);
        float f2 = ((float) (dWidth - (dSqrt * d))) / 2.0f;
    ...
    private void loadFromAttributes(@Nullable AttributeSet attributeSet, @AttrRes int i, @StyleRes int i2) {
        TypedArray typedArrayObtainStyledAttributes = ThemeEnforcement.obtainStyledAttributes(this.context, attributeSet, C3387R.styleable.Tooltip, i, i2, new int[0]);
        this.arrowSize = this.context.getResources().getDimensionPixelSize(C3387R.dimen.mtrl_tooltip_arrowSize);
        setShapeAppearanceModel(getShapeAppearanceModel().toBuilder().setBottomEdge(createMarkerEdge()).build());
        setText(typedArrayObtainStyledAttributes.getText(C3387R.styleable.Tooltip_android_text));
        setTextAppearance(MaterialResources.getTextAppearance(this.context, typedArrayObtainStyledAttributes, C3387R.styleable.Tooltip_android_textAppearance));
        setFillColor(ColorStateList.valueOf(typedArrayObtainStyledAttributes.getColor(C3387R.styleable.Tooltip_backgroundTint, MaterialColors.layer(ColorUtils.setAlphaComponent(MaterialColors.getColor(this.context, R.attr.colorBackground, TooltipDrawable.class.getCanonicalName()), TbsListener.ErrorCode.INSTALL_FROM_UNZIP), ColorUtils.setAlphaComponent(MaterialColors.getColor(this.context, C3387R.attr.colorOnBackground, TooltipDrawable.class.getCanonicalName()), TbsListener.ErrorCode.STARTDOWNLOAD_LOCKED_IO_FAILED)))));
        setStrokeColor(ColorStateList.valueOf(MaterialColors.getColor(this.context, C3387R.attr.colorSurface, TooltipDrawable.class.getCanonicalName())));
        this.padding = typedArrayObtainStyledAttributes.getDimensionPixelSize(C3387R.styleable.Tooltip_android_padding, 0);
        this.minWidth = typedArrayObtainStyledAttributes.getDimensionPixelSize(C3387R.styleable.Tooltip_android_minWidth, 0);
        this.minHeight = typedArrayObtainStyledAttributes.getDimensionPixelSize(C3387R.styleable.Tooltip_android_minHeight, 0);
        this.layoutMargin = typedArrayObtainStyledAttributes.getDimensionPixelSize(C3387R.styleable.Tooltip_android_layout_margin, 0);
    ...
    public void setRevealFraction(@FloatRange(from = 0.0d, m14515to = 1.0d) float f) {
