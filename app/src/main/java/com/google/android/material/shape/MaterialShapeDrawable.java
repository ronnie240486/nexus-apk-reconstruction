...
public class MaterialShapeDrawable extends Drawable implements TintAwareDrawable, Shapeable {
    ...
    public void getOutline(@NonNull Outline outline) {
        ...
        if (isRoundRect()) {
            C0800MO.m3734a(outline, getBounds(), getTopLeftCornerResolvedSize() * this.drawableState.interpolation);
            return;
        }
        calculatePath(getBoundsAsRectF(), this.path);
        if (C0274E7.m1537a(this.path) || Build.VERSION.SDK_INT >= 29) {
            try {
                C0337F7.m1782a(outline, this.path);
    ...
    public int getShadowOffsetX() {
        ...
        double dSin = Math.sin(Math.toRadians(materialShapeDrawableState.shadowCompatRotation));
        Double.isNaN(d);
        return (int) (dSin * d);
    }
    ...
    public int getShadowOffsetY() {
        MaterialShapeDrawableState materialShapeDrawableState = this.drawableState;
        double d = materialShapeDrawableState.shadowCompatOffset;
        double dCos = Math.cos(Math.toRadians(materialShapeDrawableState.shadowCompatRotation));
        Double.isNaN(d);
        return (int) (dCos * d);
    }
    ...
    public boolean requiresCompatShadow() {
        ...
        return i < 21 || !(isRoundRect() || C0274E7.m1537a(this.path) || i >= 29);
    }

    @Override // android.graphics.drawable.Drawable
    public void setAlpha(@IntRange(from = 0, m14516to = IjkMediaMeta.AV_CH_LAYOUT_7POINT1_WIDE_BACK) int i) {
        MaterialShapeDrawableState materialShapeDrawableState = this.drawableState;
    ...
    public static MaterialShapeDrawable createWithElevationOverlay(Context context, float f) {
        int color = MaterialColors.getColor(context, C3387R.attr.colorSurface, "MaterialShapeDrawable");
